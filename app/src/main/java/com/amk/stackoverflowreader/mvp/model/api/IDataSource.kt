...
interface IDataSource {
    ...
    fun getAllQuestions(@Query("sort") sortBy:SortedBy, @Query("order") orderBy: OrderBy): Single<RequestQuestion>

    //    @GET("/2.2/search/excerpts?order=desc&sort=relevance&site=stackoverflow")
    @GET("/2.2/search/advanced?site=stackoverflow")
    fun getFindQuestions(@Query("q") querySearch: String, @Query("sort") sortBy:SortedBy, @Query("order") orderBy: OrderBy): Single<RequestQuestion>
